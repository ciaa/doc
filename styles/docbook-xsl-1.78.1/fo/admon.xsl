  ...
  <fo:block id="{$id}"
    ...
    <xsl:choose>
      <xsl:when test="local-name($node)='note'">
         <fo:block xsl:use-attribute-sets="admonition.properties.note">
            <xsl:apply-templates/>
          </fo:block>
      </xsl:when>
      <xsl:when test="local-name($node)='warning'">
         <fo:block xsl:use-attribute-sets="admonition.properties.warning">
            <xsl:apply-templates/>
          </fo:block>
      </xsl:when>
      <xsl:when test="local-name($node)='caution'">
         <fo:block xsl:use-attribute-sets="admonition.properties.caution">
            <xsl:apply-templates/>
          </fo:block>
      </xsl:when>
      <xsl:when test="local-name($node)='tip'">
         <fo:block xsl:use-attribute-sets="admonition.properties.tip">
            <xsl:apply-templates/>
          </fo:block>
      </xsl:when>
      <xsl:when test="local-name($node)='important'">
         <fo:block xsl:use-attribute-sets="admonition.properties.important">
            <xsl:apply-templates/>
          </fo:block>
      </xsl:when>
      <xsl:otherwise>
         <fo:block xsl:use-attribute-sets="admonition.properties">foo
            <xsl:apply-templates/>
          </fo:block>
      </xsl:otherwise>
    </xsl:choose>
